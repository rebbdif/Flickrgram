...
- (id)fetchEntity:(NSString *)entity forKey:(NSString *)key context:(NSManagedObjectContext *)context {
    if (!context) context = self.stack.mainContext;
...
- (NSArray *)fetchEntities:(NSString *)entity withPredicate:(NSPredicate *)predicate context:(NSManagedObjectContext *)context {
    if (!context) context = self.stack.mainContext;
    NSFetchRequest *request = [[NSFetchRequest alloc] initWithEntityName:entity];
    request.fetchBatchSize = 30;
    request.predicate = predicate;
